...
export default function Home() {
  ...
  // creates a reference to the web3 modal (used for connecting to metamask) which persists as long as page is opened
  ...
  const getNumberOfWhitelisted = async () => {
    try {
      ...
      // call the numAddressWhitelisted from the contract
  ...
  const checkIfAddressInWhitelist = async () => {
    try {
      // we will need the signer later to get user's address
      ...
      const _joinedWhitelist =
        await whitelistContract.whitelistContract.whitelistedAddresses(address);
